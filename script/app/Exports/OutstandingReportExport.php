...
class OutstandingReportExport implements WithMapping, FromCollection, WithHeadings, WithStyles, ShouldAutoSize
{
    ...
    {
        return [
            [__('modules.expenses.reports.outstandingPaymentReport') . ' ' . Carbon::parse($this->startDate)->format(dateFormat()) .' - ' . Carbon::parse($this->endDate)->format(dateFormat())],
            [
    ...
    public function map($expense): array
    {
        return [
            ...
            $expense->last_due_date ? Carbon::parse($expense->last_due_date)->format(dateFormat()) : 'N/A', // Latest due date for pending payments
            'Pending'
        ];
    }
