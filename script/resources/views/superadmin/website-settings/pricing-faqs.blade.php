...
<div x-data="pricingFaqComponent()" x-init="initSortable()" class="p-6 md:p-10 max-w-6xl mx-auto">

    <!-- Header Section + Add New Pricing FAQ Button -->
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h2 class="text-3xl font-extrabold text-gray-900 dark:text-gray-100 tracking-tight">
                Pricing FAQs (<span x-text="pricingFaqs.length"></span>)
            </h2>
            <p class="text-sm sm:text-base text-gray-600 dark:text-gray-400 mt-1 font-medium">
                Manage pricing page specific questions. Use ▲ ▼ buttons or Drag Handle to adjust sequence order.
            </p>
        </div>
        ...
        <button @click="openCreate()"
            ...
            <span>+ Add New Pricing FAQ</span>
        </button>
    </div>

    <!-- Pricing FAQ Table Card -->
    <div class="faq-card">
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody id="pricing-faq-table-body">
                    <template x-for="(item, idx) in pricingFaqs" :key="idx">
                                ...
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <div class="flex flex-col gap-0.5">
                                        ...
                                        <button @click="moveDown(idx)"
                                                :disabled="idx === pricingFaqs.length - 1"
                                                type="button"
                                                class="faq-seq-btn"
                                                title="Move Down">
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <template x-if="pricingFaqs.length === 0">
                        <tr>
                            <td colspan="6" class="py-12 text-center text-gray-500 dark:text-gray-400 font-medium">
                                No Pricing FAQs found. Click "+ Add New Pricing FAQ" to create one.
                            </td>
                        </tr>
                    </template>
                </tbody>
        ...
        <div class="fixed inset-y-0 right-0 pl-10 max-w-full flex">
            ...
            <div x-show="openDrawer"
                ...
                <div class="p-6 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
                    <h3 class="text-xl font-bold text-gray-900 dark:text-gray-100" x-text="isEdit ? 'Update Pricing FAQ' : 'Add New Pricing FAQ'"></h3>
                ...
                <div class="p-6 space-y-6 flex-1 overflow-y-auto">
                    ...
                    <div>
                        ...
                        <input type="text"
                               x-model="question"
                               placeholder="e.g. Are there any hidden setup fees?"
                ...
                <div class="p-6 border-t border-gray-200 dark:border-gray-800 bg-gray-50/50 dark:bg-[#070A14] flex items-center justify-end gap-3">
                    ...
                    <button @click="saveFaq()" type="button" class="px-5 py-2.5 bg-skin-base text-white font-bold text-xs rounded-xl shadow-md hover:opacity-90 transition cursor-pointer">
                        <span x-text="isEdit ? 'Update Pricing FAQ' : 'Save Pricing FAQ'"></span>
                    </button>
                </div>
...
<script>
    function pricingFaqComponent() {
        return {
            ...
            answer: '',
            status: 'active',
            pricingFaqs: @json($pricingFaqs),

            initSortable() {
                this.$nextTick(() => {
                    const tbody = document.getElementById('pricing-faq-table-body');
                        ...
                        new Sortable(tbody, {
                            ...
                            onEnd(evt) {
                                if (evt.oldIndex !== undefined && evt.newIndex !== undefined && evt.oldIndex !== evt.newIndex) {
                                    const movedItem = self.pricingFaqs.splice(evt.oldIndex, 1)[0];
                                    self.pricingFaqs.splice(evt.newIndex, 0, movedItem);
            ...
            openEdit(idx) {
                this.isEdit = true;
                this.editIndex = idx;
                const item = this.pricingFaqs[idx];
                if (item) {
            ...
            saveFaq() {
                ...
                if (this.isEdit && this.editIndex !== null && this.pricingFaqs[this.editIndex]) {
                    this.pricingFaqs[this.editIndex].question = this.question;
                    this.pricingFaqs[this.editIndex].answer = this.answer;
                    this.pricingFaqs[this.editIndex].status = this.status;
                    this.pricingFaqs[this.editIndex].updated_at = now;
                } else {
                    this.pricingFaqs.push({
                        id: Date.now(),
            ...
            deleteFaq(idx) {
                ...
                if (typeof Swal !== 'undefined') {
                    Swal.fire({
                        title: 'Delete Pricing FAQ?',
                        text: 'Are you sure you want to delete this Pricing FAQ?',
                        icon: 'warning',
                        ...
                        if (result.isConfirmed) {
                            self.pricingFaqs.splice(idx, 1);
                            self.syncDb('deleted');
                        }
                    });
                } else {
                    if (confirm('Are you sure you want to delete this Pricing FAQ?')) {
                        self.pricingFaqs.splice(idx, 1);
                        self.syncDb('deleted');
                    }
                }
            ...
            toggleStatus(idx) {
                if (this.pricingFaqs[idx]) {
                    this.pricingFaqs[idx].status = this.pricingFaqs[idx].status === 'active' ? 'inactive' : 'active';
                    this.syncDb('status');
                }
            },
            ...
            moveUp(idx) {
                if (idx > 0) {
                    const temp = this.pricingFaqs[idx];
                    this.pricingFaqs[idx] = this.pricingFaqs[idx - 1];
                    this.pricingFaqs[idx - 1] = temp;
                    this.pricingFaqs = [...this.pricingFaqs];
            ...
            /* Move Sequence Down */
            moveDown(idx) {
                if (idx < this.pricingFaqs.length - 1) {
                    const temp = this.pricingFaqs[idx];
                    this.pricingFaqs[idx] = this.pricingFaqs[idx + 1];
                    this.pricingFaqs[idx + 1] = temp;
                    this.pricingFaqs = [...this.pricingFaqs];
            ...
            syncDb(actionType = 'saved') {
                fetch('{{ route('superadmin.website-settings.pricing-faqs.store') }}', {
                    method: 'POST',
                    headers: {
                    ...
                    body: JSON.stringify({ pricing_faqs: this.pricingFaqs })
                })
                .then(res => res.json())
                .then(data => {
                    if (data.success) {
                        if (typeof Swal !== 'undefined') {
                            let msg = 'Pricing FAQ saved successfully.';
                            if (actionType === 'deleted') msg = 'Pricing FAQ deleted successfully.';
                            if (actionType === 'status') msg = 'Pricing FAQ status updated successfully.';
                            if (actionType === 'reordered') msg = 'Pricing FAQ sequence updated successfully.';
                        ...
                        if (typeof Swal !== 'undefined') {
                            Swal.fire('Error', data.message || 'Failed to update Pricing FAQs.', 'error');
                        }
                    }
                })
                .catch(err => {
                    if (typeof Swal !== 'undefined') {
                        Swal.fire('Error', 'Failed to sync Pricing FAQs with database.', 'error');
                    }
                });
            }
        };
