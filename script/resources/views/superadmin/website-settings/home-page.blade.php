...
<style>
    ...
    /* Stat Cards & Labels (Dark Mode Fix) */
    .hp-stat-card {
        padding: 0.875rem;
        ...
        <div class="hp-card space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <template x-if="hero.dashboard_image">
                            <div class="mt-3 relative group inline-block w-full">
                                <img :src="hero.dashboard_image" class="max-h-40 w-full object-contain rounded-xl border border-gray-300 dark:border-gray-700 shadow-sm bg-white dark:bg-gray-900 p-1">
                                <button @click="hero.dashboard_image = ''" type="button" title="Remove Image" class="absolute -top-2 -right-2 w-6 h-6 bg-red-500 hover:bg-red-600 text-white rounded-full flex items-center justify-center text-xs opacity-90 group-hover:opacity-100 transition shadow-md cursor-pointer">✕</button>
                            </div>
                        </template>
                    </div>
                    <div>
            ...
            <div>
                ...
                <div class="space-y-4">
                    ...
                    <template x-if="hero.trusted_by_logos && hero.trusted_by_logos.length > 0">
                        <div class="mt-4 pt-3 border-t border-gray-200 dark:border-gray-800">
                            <label class="block text-xs font-bold text-gray-600 dark:text-gray-300 mb-3">Uploaded Logos (<span x-text="hero.trusted_by_logos.length"></span>):</label>
                            <div class="flex flex-wrap items-center gap-4">
                                <template x-for="(logo, lIdx) in hero.trusted_by_logos" :key="lIdx">
                                    <div class="relative group p-2.5 bg-white dark:bg-gray-900 rounded-xl border border-gray-300 dark:border-gray-700 shadow-xs flex items-center justify-center w-28 h-20 shrink-0">
                                        <img :src="logo" class="max-h-14 max-w-full object-contain">
                                        <button @click="removeBrandLogo(lIdx)" type="button" title="Remove Logo" class="absolute -top-2 -right-2 w-6 h-6 bg-red-500 hover:bg-red-600 text-white rounded-full flex items-center justify-center text-xs opacity-90 group-hover:opacity-100 transition shadow-md cursor-pointer">✕</button>
                                    </div>
                                </template>
                            </div>
                        </div>
