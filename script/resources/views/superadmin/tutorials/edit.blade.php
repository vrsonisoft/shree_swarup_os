...
<div class="p-6">
    <div class="mb-6 flex items-center gap-3">
        ...
        <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-200">Edit Tutorial</h2>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg max-w-4xl">
        <div class="p-6">
            <form action="{{ route('superadmin.tutorials.update', $tutorial->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-input id="title" name="title" type="text" class="mt-1 block w-full" value="{{ old('title', $tutorial->title) }}" required />
                        ...
                        <div>
                            ...
                            <select id="tutorial_category_id" name="tutorial_category_id"
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('tutorial_category_id', $tutorial->tutorial_category_id) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="tutorial_sub_category_id" name="tutorial_sub_category_id"
                                ...
                                @foreach ($subCategories as $subCat)
                                    <option value="{{ $subCat->id }}" data-category="{{ $subCat->tutorial_category_id }}" {{ old('tutorial_sub_category_id', $tutorial->tutorial_sub_category_id) == $subCat->id ? 'selected' : '' }}>
                                        {{ $subCat->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-input id="slug" name="slug" type="text" class="mt-1 block w-full" value="{{ old('slug', $tutorial->slug) }}" required />
                        ...
                        <div>
                            ...
                            @if ($tutorial->thumbnail)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $tutorial->thumbnail) }}" alt="Thumbnail" class="w-32 h-20 object-cover rounded border dark:border-gray-700">
                                </div>
                            @endif
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <x-input id="video_title" name="video_title" type="text" class="mt-1 block w-full" value="{{ old('video_title', $tutorial->video_title) }}" />
                            <x-input-error for="video_title" class="mt-2" />
                        </div>
                        <div>
                            <x-label for="video_duration" value="Video Duration (Optional)" />
                            <x-input id="video_duration" name="video_duration" type="text" class="mt-1 block w-full" value="{{ old('video_duration', $tutorial->video_duration) }}" />
                            <x-input-error for="video_duration" class="mt-2" />
                        </div>
                        <div>
                            <x-label for="youtube_url" value="YouTube Video URL (Optional)" />
                            <x-input id="youtube_url" name="youtube_url" type="url" class="mt-1 block w-full" value="{{ old('youtube_url', $tutorial->youtube_url) }}" placeholder="https://www.youtube.com/watch?v=..." />
                    ...
                    <div>
                        ...
                        <textarea id="short_description" name="short_description" rows="3"
                            class="mt-1 block w-full bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 border-gray-300 dark:border-gray-700 focus:border-skin-base focus:ring-skin-base rounded-md shadow-sm"
                            required>{{ old('short_description', $tutorial->short_description) }}</textarea>
                    ...
                    <div>
                        ...
                        <div class="mt-2">
                            <input id="full_description" name="full_description" type="hidden" value="{{ old('full_description', $tutorial->full_description) }}">
