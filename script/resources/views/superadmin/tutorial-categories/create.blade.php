...
<div class="p-6">
    <div class="mb-6 flex items-center gap-3">
        ...
        <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-200">Add Tutorial Category</h2>
    </div>

    <div class="max-w-3xl bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
        <div class="p-6">
            <form action="{{ route('superadmin.tutorial-categories.store') }}" method="POST">
                @csrf
                <div class="space-y-6">
                    <div>
                        <x-label for="name" value="Category Name" />
                        <x-input id="name" name="name" type="text" class="mt-1 block w-full" value="{{ old('name') }}" placeholder="e.g. Dashboard & Setup" required />
                        <x-input-error for="name" class="mt-2" />
                    </div>
                    <div>
                        <x-label for="slug" value="Slug (Optional)" />
                        <x-input id="slug" name="slug" type="text" class="mt-1 block w-full" value="{{ old('slug') }}" placeholder="e.g. dashboard-setup" />
                        <x-input-error for="slug" class="mt-2" />
                        <p class="text-xs text-gray-400 mt-1">If left blank, slug will be auto-generated from the name.</p>
                    </div>
                    <div>
                        ...
                        <textarea id="description" name="description" rows="4"
                            ...
                            placeholder="Brief details about this category...">{{ old('description') }}</textarea>
