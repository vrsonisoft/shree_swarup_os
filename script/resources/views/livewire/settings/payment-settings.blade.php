    ...
    <div
        ...
        @else
            ...
            <div
                ...
                @if ($isGlobalTlyncEnabled)
                    <li wire:click="activeSetting('tlync')" class="min-w-0">
                        <span @class([
                            'inline-flex w-full min-w-0 items-center justify-center gap-1.5 cursor-pointer select-none border-b-2 px-1.5 py-3 text-center text-xs leading-tight sm:px-2 sm:text-sm rounded-t-lg hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300',
                            'border-transparent' => $activePaymentSetting != 'tlync',
                            'active border-skin-base dark:text-skin-base dark:border-skin-base text-skin-base' =>
                                $activePaymentSetting == 'tlync',
                        ])>
                            <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                                <path d="M4 4h16v16H4V4zm2 2v12h12V6H6zm2 2h8v2H8V8zm0 4h8v2H8v-2z"/>
                            </svg>
                            @lang('modules.billing.tlync')
                            <span @class([
                                'flex h-2.5 w-2.5 shrink-0 rounded-full',
                                'bg-green-500' => $isTlyncEnabled,
                                'bg-red-500' => !$isTlyncEnabled,
                            ])></span>
                        </span>
                    </li>
                @endif
        ...
        @if($activePaymentSetting == 'tlync')
            <form wire:submit="submitFormTlync">
                <div class="grid gap-6">
                    <div class="my-3">
                        <x-label for="tlyncStatus">
                            <div class="flex items-center cursor-pointer">
                                <x-checkbox name="tlyncStatus" id="tlyncStatus" wire:model.live='tlyncStatus' />
                                <div class="ms-2">
                                    @lang('modules.settings.enableTlync')
                                </div>
                            </div>
                        </x-label>
                    </div>
                    ...
                    @if ($tlyncStatus)
                        <div>
                            <x-label for="tlyncMode" :value="__('modules.settings.selectEnvironment')" required/>
                            <x-select id="tlyncMode" class="block w-full mt-1" wire:model.live="tlyncMode">
                                <option value="test">@lang('app.test')</option>
                                <option value="live">@lang('app.live')</option>
                            </x-select>
                            <x-input-error for="tlyncMode" class="mt-2"/>
                        </div>

                        @if ($tlyncMode == 'live')
                            <div>
                                <x-label for="liveTlyncStoreId" :value="__('modules.settings.liveTlyncStoreId')" required/>
                                <x-input id="liveTlyncStoreId" class="block w-full mt-1" type="text" wire:model='liveTlyncStoreId'/>
                                <x-input-error for="liveTlyncStoreId" class="mt-2"/>
                            </div>

                            <div>
                                <x-label for="liveTlyncStoreToken" :value="__('modules.settings.liveTlyncStoreToken')" required/>
                                <x-input-password id="liveTlyncStoreToken" class="block w-full mt-1" type="text" wire:model='liveTlyncStoreToken'/>
                                <x-input-error for="liveTlyncStoreToken" class="mt-2"/>
                            </div>
                        @else
                            <div>
                                <x-label for="testTlyncStoreId" :value="__('modules.settings.testTlyncStoreId')" required/>
                                <x-input id="testTlyncStoreId" class="block w-full mt-1" type="text" wire:model='testTlyncStoreId'/>
                                <x-input-error for="testTlyncStoreId" class="mt-2"/>
                            </div>

                            <div>
                                <x-label for="testTlyncStoreToken" :value="__('modules.settings.testTlyncStoreToken')" required/>
                                <x-input-password id="testTlyncStoreToken" class="block w-full mt-1" type="text" wire:model='testTlyncStoreToken'/>
                                <x-input-error for="testTlyncStoreToken" class="mt-2"/>
                            </div>
                        @endif
                        ...
                        <div class="mt-4">
                            ...
                            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">@lang('modules.settings.tlyncWebhookHelp')</p>
                        </div>
                    @endif
