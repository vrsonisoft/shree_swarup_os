...
<div class="min-w-0 max-w-full overflow-x-hidden">
    ...
    @if ($roomQrOrderingBlocked ?? false)
        <div class="px-4 mt-4">
            <div class="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-100" role="alert">
                @lang('messages.roomQrRoomServiceDisabled')
            </div>
        </div>
    @endif
    ...
    @if (!$showCart)
        ...
        @if ((!$cameFromTableQR || !$tableQrOrderingBlocked) && (!$cameFromRoomQR || !$roomQrOrderingBlocked))
                             ...
                             <div class="flex min-w-0 w-full gap-3 p-3 sm:gap-4">
                                ...
                                <div
                                    ...
                                    <div class="flex items-center justify-between w-full">
                                        ...
                                        @if ($canCreateOrder && !($tableQrOrderingBlocked ?? false) && !($roomQrOrderingBlocked ?? false))
                                            @if (!$item->in_stock)
                ...
                <div>
                    ...
                    <div class="w-full h-auto pt-3 pb-4 text-center select-none"
                        ...
                        <div class="flex gap-2">
                            ...
                            @elseif (is_null($customer) && ($restaurant->customer_login_required || in_array($orderTypeSlug, ['delivery', 'pickup'])))
    ...
    <x-dialog-modal wire:model.live="showCustomerNameModal" maxWidth="sm">
        <x-slot name="title">
            @if ($pendingTlyncOrderId)
                @lang('messages.tlyncEnterPhoneTitle')
            @else
                @lang('modules.customer.enterName')
            @endif
        </x-slot>

        <x-slot name="content">
            @if (!is_null($customer) || $pendingTlyncOrderId)
                <form wire:submit="submitCustomerName">
                    @csrf
                    <div class="space-y-4">
                        @if ($pendingTlyncOrderId)
                            <p class="text-sm text-gray-600 dark:text-gray-400">
                                @lang('messages.tlyncPhoneHelp')
                            </p>
                        @endif

                        @if (!$pendingTlyncOrderId || empty($customerName))
                        <div>
                        ...
                        <div>
                            ...
                            <div class="flex gap-2 mt-1">
                                @if ($pendingTlyncOrderId)
                                    <div class="flex items-center px-3 py-2 text-sm bg-gray-100 border border-gray-300 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300">
                                        +218
                                    </div>
                                @else
                                <!-- Phone Code Dropdown -->
                                ...
                                <x-input id="customerPhone" class="block w-full" type="tel"
                                    wire:model='customerPhone' placeholder="{{ $pendingTlyncOrderId ? '0912345678' : '1234567890' }}" />
                            </div>
                    ...
                    <div class="grid items-center w-full grid-cols-1 gap-6 mt-4 md:grid-cols-2">
                        @if ($paymentGateway->stripe_status)
                            <x-secondary-button
                                ...
                                wire:loading.attr="disabled"
                                wire:loading.class="opacity-50 cursor-not-allowed"
                                wire:target="initiatePayment,initiateStripePayment,initiateFlutterwavePayment,initiatePaypalPayment,initiatePayfastPayment,initiatePaystackPayment,initiateXenditPayment,initiateEpayPayment,initiateMolliePayment,initiateTapPayment,initiateTlyncPayment,placeOrder">
                        ...
                        @if ($paymentGateway->tlync_status)
                            <x-secondary-button
                                wire:click="initiateTlyncPayment({{ $paymentOrder?->id ?: 'null' }})"
                                wire:loading.attr="disabled"
                                wire:loading.class="opacity-50 cursor-not-allowed"
                                wire:target="initiatePayment,initiateStripePayment,initiateFlutterwavePayment,initiatePaypalPayment,initiatePayfastPayment,initiatePaystackPayment,initiateXenditPayment,initiateEpayPayment,initiateMolliePayment,initiateTapPayment,initiateTlyncPayment,placeOrder">
                                <span class="inline-flex items-center">
                                    <svg class="w-4 h-4 ltr:mr-1 rtl:ml-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
