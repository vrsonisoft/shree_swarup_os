        ...
        <div class="flex flex-col justify-between gap-3 mb-3 md:flex-row">
            <div class="flex flex-col gap-1.5">
                ...
                <div class="flex flex-wrap gap-3 text-xs text-gray-600 dark:text-gray-400">
                    ...
                    @endif
                    @if (function_exists('isHotelModuleEnabled') && isHotelModuleEnabled($restaurant) && $order->context_room_number)
                        <div class="flex items-center gap-1.5">
                            <svg class="w-3.5 h-3.5 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/>
                            </svg>
                            @lang('hotel::modules.folio.room')
                            <span class="px-1.5 py-0.5 bg-skin-base text-gray-100 rounded-full text-xs font-medium">
                                {{ $order->context_room_number }}
                            </span>
                        </div>
                    @endif
                </div>
                ...
                <div class="flex gap-4">
                    ...
                    @else
                        <div class="grid w-full grid-cols-3 gap-4">
                            @php
                                $showPayNow = $paymentGateway->is_qr_payment_enabled || $paymentGateway->stripe_status || $paymentGateway->razorpay_status || $paymentGateway->flutterwave_status || $paymentGateway->paypal_status || $paymentGateway->payfast_status || $paymentGateway->paystack_status || $paymentGateway->xendit_status || $paymentGateway->epay_status || $paymentGateway->mollie_status || $paymentGateway->tap_status || $paymentGateway->tlync_status || count($offlinePaymentMethods) > 0;
                            @endphp
                            @if ($showPayNow)
                ...
                @else
                    ...
                    <div class="grid items-center w-full grid-cols-1 gap-4 mt-4 md:grid-cols-2">
                        ...
                        @if ($paymentGateway->tlync_status)
                            <x-secondary-button wire:click='initiateTlyncPayment({{ $paymentOrder->id }})'>
                                <span class="inline-flex items-center">
