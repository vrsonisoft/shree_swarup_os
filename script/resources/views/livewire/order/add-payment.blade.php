    ...
    <x-dialog-modal wire:model.live="showAddPaymentModal" maxWidth="3xl" class="w-full">
        <x-slot name="title">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-0">
                ...
                <div class="flex items-center gap-2 sm:gap-4 mt-2 sm:mt-0">
                    ...
                    <span class="text-lg sm:text-xl font-bold text-skin-base">
                        {{ currency_format($paymentMethod === 'due' ? $this->amountOnDue : $order->total, restaurant()->currency_id) }}
                    </span>
                </div>
            </div>
            @endif
                    ...
                    <div class="w-full md:flex-1">
                        ...
                        <div @class([
                            ...
                            <button wire:click="setPaymentMethod('upi')"
                                class="min-h-[92px] p-3 text-center border rounded-lg flex flex-col items-center justify-center {{ $paymentMethod === 'upi' ? 'bg-skin-base/5 border-skin-base' : 'hover:bg-gray-50' }}">
                                ...
                                <span class="text-sm leading-tight">@lang('modules.order.upi')</span>
                            </button>
                            ...
                            @if (module_enabled('StripeTerminal'))                                
                                {{-- STRIPE_TERMINAL_POS:BEGIN --}}
                                @includeIf('stripeterminal::sections.pos.payment-methods')
                        ...
                        <div class="mt-4 space-y-4">
                            <div>
                                <label class="block text-sm mb-1">
                                    {{ $paymentMethod === 'due' ? __('modules.order.amountPaid') : __('modules.order.amount') }}
                                </label>
                                ...
                                @if($paymentMethod === 'due')
                                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                        ...
                                        <span class="font-semibold text-red-600 dark:text-red-400">{{ currency_format($this->amountOnDue, restaurant()->currency_id) }}</span>
                                    </p>
                                @endif
                            </div>
                            ...
                            <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 my-2 text-sm sm:text-base">
                                <div class="space-y-2">
                                    ...
                                    <div class="flex justify-between text-red-600 pt-2 border-t">
                                        ...
                                        <span class="font-medium">{{ currency_format($this->amountOnDue, restaurant()->currency_id) }}</span>
                                    </div>
                                    @elseif($returnAmount > 0)
        ...
        <x-slot name="footer">
            ...
            <div class="flex flex-row w-full gap-3">
                <x-button-cancel class="flex-1 px-6 py-3 text-base font-medium text-gray-700 hover:bg-gray-100 rounded-lg" wire:click="$toggle('showAddPaymentModal')" wire:loading.attr="disabled" wire:target="submitForm,submitFormAndPrint">
                    {{ __('app.cancel') }}
                </x-button-cancel>
